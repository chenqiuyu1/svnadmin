...
class IF_SVNAuthFileC
{
     ...
     * @return bool
     */
    public function repositoryPathMatchExists($repository)
    {
        return $this->config->getSectionMatchExists($repository);
    }
     ...
     * @throws Exception If an invalid repository path has been provided.
     */
    public function cpRepositoryPath($orepopath,$drepopath)
    {
        $match_sections = self::repositoryPathMatchExists($orepopath);
        if (!$match_sections)
        {
            // Already exists.
            return false;
        ...
        // Validate the $repopath string.
        $pattern = '/^[A-Za-z0-9\_\-]+:\/.*$/i';
        if ($orepopath != "/" && !preg_match($pattern, $orepopath))
        {
            throw new Exception('Invalid repository name. (Pattern: '.$pattern.')');
        }
        if ($drepopath != "/" && !preg_match($pattern, $drepopath))
        {
            throw new Exception('Invalid repository name. (Pattern: '.$pattern.')');
        }
        //Handle match repopath
        foreach ($match_sections as $m){
            $repovalues = $this->config->getSectionValues($m);
            $tmpdrepopath = str_replace($orepopath,$drepopath,$m);
            // Copy the branch configuration path.
            $this->config->setValueArray($tmpdrepopath,$repovalues);
        }
        return true;
    }
