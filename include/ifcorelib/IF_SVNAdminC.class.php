...
class IF_SVNAdminC extends IF_SVNBaseC
{
	...
	public function create($path, $type="fsfs")
	{
		...
		//bug修改添加
		exec('/usr/bin/sudo /bin/chmod -R 777 '.self::encode_local_path($path));
		if ($exitCode != 0)
		{
     ...
     * @throws IF_SVNCommandExecutionException
     */
    public function copy($opath,$dpath,$type="fsfs")
    {
        ...
        $despath = self::encode_local_path($opath).' '.self::encode_local_path($dpath);
        $cmd = self::create_svn_command($this->m_svnadmin, "cp",$despath ,$args, false);
	...
	{
		exec('/usr/bin/sudo /bin/chmod -R 777 '.$path);
