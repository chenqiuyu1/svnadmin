...
class IF_Config
{
     ...
     */
    public function setValueArray($section, $arr)
    {
        if (!isset($this->items[$section]))
        {
            $this->items[$section] = array();
    ...
    /**
     * Gets to know whether a specific match section exists.
     *
     * @param string $section
     *
     * @return bool
     */
    public function getSectionMatchExists($section)
    {
        $match_sections = array();
        $sections = $this->getSections();
        foreach ($sections as $s){
            $istrue = preg_match('@^'.$section.'($|/)@',$s);
            //if(strpos($s,$section) !== false ){
            if($istrue){
                $match_sections[] = $s;
            }
        }
    ...
    {
        return $this->items[$section] ? $this->items[$section] : array();
    }
