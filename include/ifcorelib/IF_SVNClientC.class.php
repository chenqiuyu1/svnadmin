...
class IF_SVNClientC extends IF_SVNBaseC
{
     ...
     *
     * @param string $opath Absolute path to the repository
     * @param string $dpath Absolute path to the repository
     * @param string $type Repository type: fsfs=file system(default); bdb=berkley db (not recommended)
     * @throws IF_SVNException
     * @throws IF_SVNCommandExecutionException
     */
    public function copy($opath,$dpath,$msg='create-branch')
    {
        if (empty($opath) || empty($dpath))
        {
        ...
        $pattern = '/^([a-z0-9\_\-]+)$/i';
        $orepo_name = basename($opath);
        $drepo_name = basename($dpath);

        if (!preg_match($pattern, $orepo_name) || !preg_match($pattern, $drepo_name))
        {
            throw new IF_SVNException('Invalid repository name: '.$orepo_name. ' ' . $drepo_name . ' (Allowed pattern: '.$pattern.')');
        }
        ...
        $despath = self::encode_url_path($opath).' '.self::encode_url_path($dpath);
        $cmd = self::create_svn_command($this->svnExe, "cp",$despath ,$args, false);
        ...
        if ($exitCode != 0)
        {
            throw new IF_SVNCommandExecutionException('Command='.$cmd.'; Return='.$exitCode.'; Output='.var_export($output,1).';');
        }
    }
	/**
     ...
     * @throws IF_SVNOutputParseException
     */
    public function svn_islist($path, &$outList = null)
    {
        if (empty($path))
        {
            throw new IF_SVNException('Empty path-parameter for svn_islist() command.');
        }
