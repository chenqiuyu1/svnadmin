...
try {
    ...
    $JerryBranch = $engine->getConfig()->getValue("GUI", "JerryBranch");
	$hasJerryBranch = !empty($JerryBranch) ? true : false;
	...
	foreach ($repoPathList as &$val)
	{
		...
		{
			//浏览bug修复
			$args = array($oR->getEncodedName(), $val->getRelativePath());
    ...
    SetValue("JerryBranch", $hasJerryBranch);
